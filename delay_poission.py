...
arrival_lambda = [0.002, 0.0018, 0.0028, 0.004]
...
next_arrival = [[0 for col in range(4)] for row in range(3)]
channel_active = [[0 for col in range(4)] for row in range(3)]
...
def Arrival(m, n, k):
    tmp = k + \
        int(math.ceil(-math.log(1.0 - random.random()) / arrival_lambda[n]))
    next_arrival[m][n] = tmp


def Channel(m, n, k):
    tmp = k + \
        int(math.ceil(-math.log(1.0 - random.random()) / channel_lambda[n]))
    channel_active[m][n] = tmp
...
for h in range(0, loops):
    ...
    for i in range(0, count - 1):
        if i == 0:
            for m in range(0, 3):
                for n in range(0, 4):
                    next_arrival[m][n] = 1
                    channel_active[m][n] = 1

        for m in range(0, 3):
            for n in range(0, 4):
                if channel_active[m][n] == i:
                    channel[m][n] = 1
                    Channel(m, n, i)
        ...
        w[1][0] = channel[1][1] * hol[1] * k[1] + channel[1][3] * hol[3] * k[3]
        w[1][1] = channel[1][0] * hol[0] * k[0] + channel[1][2] * hol[2] * k[2]
        w[1][2] = channel[1][0] * hol[0] * k[0] + channel[1][3] * hol[3] * k[3]
    ...
    for m in range(0, 3):
        if through_count[m] != 0:
            ...
            queue_length[m][h] = 0  # queue_length[m][max(h - 1, 0)]
    Clear_queue()
    for m in range(0, 3):
        through_count[m] = 0
        delay_rec[m] = 0.0
...
tmp = [0, 0, 0]
plt.figure(1)
x = np.linspace(0.002, 0.802, loops)
