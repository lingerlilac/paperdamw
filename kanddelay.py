...
arrival_lambda = [0.82, 0.018, 0.58, 0.34]
...
for h in range(0, loops):
    ...
    for m in range(0, 3):
        if through_count[m] != 0:
            ...
            queue_length[m][h] = queue_length[m][max(h - 1, 0)] / 2
    #for m in range(0, 3):
        # for n in range(0, 4):
...
plt.xlabel('Input rate (0.82(x-axis), 0.41, 0.82, 0.41)')
