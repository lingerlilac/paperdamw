...
alp = 0.9
for n in range(0, 2):
    q[0][n] = Queue.Queue(maxsize=count)
...
def Arrival(i):
    for m in range(0, 2):
        if R[m] == 1 and q[1][m].full() is False:
            q[1][m].put(i)
            arrival[1][m] += 1
        elif q[1][m].full() is True:
            print('queue', 2 + m, 'is full')
        tmp = np.random.binomial(1, arrival_lambda[m], 1)
        if q[0][m].full() is False and tmp[0] == 1:
            q[0][m].put(i)
            arrival[0][m] += 1
...
for j in range(0, loops):
    ...
    for i in range(0, count):
        ...
        if q[0][0].empty() is False:
            tm1 = i - q[0][0].queue[0]
        else:
            tm1 = 0
        if q[0][1].empty() is False:
            tm2 = i - q[0][1].queue[0]
        else:
            tm2 = 0
        ...
        for m in range(0, 2):
            ...
            if Z[0][m] <= tmp:
                drop_decision[m] = 1
        tmp1 = q[1][0].qsize() * channel[1][0] + (Z[1][0] - H[1][0]) * \
            (channel[1][0] * k[0] * w[0] - alpha)
        tmp2 = q[1][1].qsize() * channel[1][1] + (Z[1][1] - H[1][1]) * \
            (channel[1][1] * k[1] * w[1] - alpha)
        if tmp1 > tmp2:
            schedule[1] = 0
        elif tmp1 == tmp2:
            tmp = [0, 1]
        ...
        if q[0][schedule[0]].empty() is False and channel[0][schedule[0]] == 1:
            ...
            if (i - tmp2) > max_delay[0][schedule[0]]:
                max_delay[0][schedule[0]] = i - tmp2
            # print('there')
        ...
        for m in range(0, 2):
            ...
            if tmp > 0:
                ...
                Z[0][m] = 0.0
        for m in range(0, 2):
            tmp = Z[1][m] - k[m] * channel[1][m] * w[m] + alpha
            if tmp > 0:
                Z[1][m] = tmp
            else:
                Z[1][m] = 0
        for m in range(0, 2):
            tmp = H[1][m] + k[m] * channel[1][m] * w[m] - alpha
            if tmp > 0:
                H[1][m] = tmp
            else:
                H[1][m] = 0
